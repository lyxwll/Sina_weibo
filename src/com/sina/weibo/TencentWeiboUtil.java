...
public class TencentWeiboUtil {
	...
	public void initTencentWeibo(WeiboListener l) {
		...
		if (TextUtils.isEmpty(accessToken)) {
			...
			if (expiresTime - System.currentTimeMillis() > 0) {
				...
				String clientId = PreferenceUtil.getInstance(mContext).getString(
						Constants.PREF_TX_CLIENT_ID, "");
	...
	public void auth(final WeiboListener l) {
		...
		long appId = Long.valueOf(com.tencent.weibo.sdk.android.api.util.Util.getConfig()
				.getProperty(Constants.TX_APP_KEY));
		String appSecket = com.tencent.weibo.sdk.android.api.util.Util.getConfig().getProperty(
				Constants.TX_APP_KEY_SEC);
		listener = l;
		...
		AuthHelper.register(mContext, appId, appSecket, new OnAuthListener() {
			...
			public void onAuthPassed(String name, WeiboToken token) {
				...
				String clientId = com.tencent.weibo.sdk.android.api.util.Util.getConfig()
						.getProperty(Constants.TX_APP_KEY);
				String clientIp = getClientIp();
				PreferenceUtil.getInstance(mContext).saveString(Constants.PREF_TX_ACCESS_TOKEN,
						token.accessToken);
				PreferenceUtil.getInstance(mContext).saveString(Constants.PREF_TX_EXPIRES_IN,
						String.valueOf(token.expiresIn));
				PreferenceUtil.getInstance(mContext).saveString(Constants.PREF_TX_OPEN_ID,
						token.openID);
				PreferenceUtil.getInstance(mContext).saveString(Constants.PREF_TX_OPEN_KEY,
						token.omasKey);
				PreferenceUtil.getInstance(mContext).saveString(Constants.PREF_TX_REFRESH_TOKEN,
						token.refreshToken); // 总是为null
				PreferenceUtil.getInstance(mContext).saveString(Constants.PREF_TX_CLIENT_ID,
						clientId);
				...
				PreferenceUtil.getInstance(mContext).saveString(Constants.PREF_TX_CLIENT_IP,
						clientIp);
	...
	public void addWeibo(String content, long longitude, long latitude, int syncflag,
		...
		weiboAPI.addWeibo(content, longitude, latitude, syncflag, compatibleflag,
				new RequestListener() {
	...
	public void uploadPics() {
		...
		TencentWeiboAPI weiboAPI = new TencentWeiboAPI(tencentTO);
		weiboAPI.uploadPic("test", "/storage/emulated/0/ushow/show/1409620917071",
				"/storage/emulated/0/ushow/show/1409620917071", 2, new RequestListener() {
	...
	public void addPic(String content, long longitude, long latitude, String pic, int syncflag,
			int compatibleflag, int empty) {

		TencentWeiboAPI weiboAPI = new TencentWeiboAPI(tencentTO);
		weiboAPI.addPic(content, longitude, latitude, pic, syncflag, compatibleflag, empty,
				new RequestListener() {
