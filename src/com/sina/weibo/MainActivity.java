...
public class MainActivity extends Activity {
	...
	private void initBtnAddTX() {
		...
		mBtnAddTX.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				// TencentWeiboUtil.getInstance(mContext).addWeibo("腾讯微博开放平台，Hello world！～",
				// 0, 0, 0, 0);
				TencentWeiboUtil.getInstance(mContext).addPic("test", 0, 0,
						"/storage/emulated/0/ushow/show/1409620917071", 0, 0, 1);
	...
	private void refreshView() {
		...
		LOG.cstdr("refreshUserView", "sinaToken = " + sinaToken + "   tencentToken = "
				+ tencentToken);
		...
		if (TextUtils.isEmpty(sinaToken) && TextUtils.isEmpty(tencentToken)) {
			...
			if (!TextUtils.isEmpty(tencentToken)) {
				// 初始化腾讯微博，判断是否授权过期
				TencentWeiboUtil.getInstance(mContext).initTencentWeibo(new WeiboListener() {
					...
					public void init(boolean isValid) {
						...
						if (isValid) {
							...
							String name = PreferenceUtil.getInstance(mContext).getString(
